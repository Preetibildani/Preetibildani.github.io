...
const Skills = () => {
    ...
    return (
        <Box bg={bg} color={color} paddingTop={"80px"} id="Skills">
            <Text textAlign={"center"} fontSize="5xl" color={"#5fc49a"} paddingY={"20px"}>
                Skills
            </Text>
            <SimpleGrid width={"60%"} margin={"auto"} columns={[1, 2, 2, 3]} padding={"20px"} justifyContent={"space-between"} alignItems={"start"} gap={"20px"}>
                <Fade top cascade >
                    <VStack gap={'10px'}>
                        <Text bg={bg} textAlign={"center"} fontSize="2xl" color={"white"} paddingY={"20px"}>
                            Frontend Skills
                        </Text>
                        {(bg === 'white' ? frontendBlue : frontendYellow).map((elem) => {
                            return <Box borderColor={"white"} className='hvr-grow-shadow' alignItems={"center"} borderRadius={"10px"} gap={'20px'} display={"flex"} width={"100%"} bg={color} padding={"15px"} color={"white"} key={elem.title}> <Img width={"11%"} src={elem.image} alt={elem.alt} /> <Text fontSize={'xl'}> {elem.alt} </Text> </Box>
                        })}
                    </VStack>
                </Fade>
                <Fade top cascade >
                    <VStack gap={'10px'}>
                        ...
                        {(bg === 'white' ? backendBlue : backendYellow).map((elem) => {
                            return <Box borderColor={bg} className='hvr-grow-shadow' alignItems={"center"} borderRadius={"10px"} gap={'20px'} display={"flex"} width={"100%"} bg={color} padding={"15px"} color={"white"} key={elem.title}> <Img width={"11%"} src={elem.image} alt={elem.alt} /> <Text fontSize={'xl'}> {elem.alt} </Text> </Box>
                        })}
                    </VStack>
                </Fade>
                <Fade top cascade >
